...
def build_llm_prompt(prompt_text: str, params: Parameters) -> str:
    ...
    blender_rules = (
        ...
        f"3. MATERIALS: Call `set_material('MatName', R, G, B, {m.roughness}, {m.metallic}, 'ObjectName')` to color specific parts (e.g. 'Foliage', 'Trunk'). If 'ObjectName' is omitted, it colors all uncolored parts.\n\n"
        f"4. CONCISENESS: Do NOT define `make_box`, `make_cylinder`, `make_sphere`, `make_cone`, `make_torus`, `set_material`, `build_chair`, or `build_table`. They are already defined for you. Just call them directly! Avoid comments and empty lines.\n\n"
